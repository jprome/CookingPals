  ...
  return (
    <ThemeProvider theme={theme}>
      <Grid
        container
        spacing={1}
        sx={{ backgroundColor: "#EEEEEE33" }}
        component="main"
      >
        <Grid item xs={12} sx={{ pl: 10, ml: 10, pt: 10, mt: 10, pb: 5 }}>
          <Typography fontFamily="Helvetica" variant="h2">
            Welcome back!
          </Typography>
        </Grid>
        <Grid item xs={12} sx={{ pl: 10, ml: 10, pt: 10, mt: 10, pb: 5 }}>
          <Typography fontFamily="Helvetica" variant="h4">
            Today's Best Recipes
          </Typography>
        </Grid>
        ...
        <Grid item xs={12} sx={{ pl: 20, ml: 10, mt: 10 }}>
          <Typography fontFamily="Helvetica" variant="h4">
            CookingPal Recommendations
          </Typography>
        </Grid>

        <Grid container sx={{ ml: 10, mr: 10, mt: 4 }}>
          {searchRequest.users ? (
            searchRequest.users!.map((user) => {
              return (
                <Grid item key={`${user._id}Grid4`} xs={6}>
        ...
        <Grid item xs={12} sx={{ m: 10, p: 10 }}>
          {auth.user ? (
            auth.user!.friendRequestReceived.map((n: any) => {
              return (
                <Box sx={{ width: "750px", ml: 10 }}>
                  <Grid
                    container
                    xs={12}
                    sx={{

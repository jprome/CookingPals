...
const SectionComponent = (s: SectionProps) => {
  if (s.section === 0) {
    return (
      <React.Fragment>
        <Grid container spacing={0} rowSpacing={0}></Grid>
      </React.Fragment>
    );
  }
  if (s.section === 1) {
...
export default function CookbookPage() {
  ...
  const [alert_open, setAlertOpen] = React.useState({
    state: false,
    message: "",
  });
  ...
  const cookbookStateSample = {
    ...
    cookbook: {
      ...
      description: "",
      recipes: [] as any,
      diet_filters: [] as string[],
    },
  };
  const [cookbookState, setCookbookState] =
    useState<IState>(cookbookStateSample);
  const { cookbook, own } = cookbookState;

  //State for editing current cookbook
  const [cookbookEditState, setCookbookEditState] = useState({
    _id: "",
    title: "",
    description: "",
    recipes: [] as any,
  ...
  const handleDialogOpen = (n: number, a?: any, index?: number) => {
    if (n === 0) {
      ...
      const temp_diet_filters = cookbook.diet_filters
        ? cookbook.diet_filters
        : ([] as string[]);
  ...
  const handleDietChange = (diet: string, n?: number) => {
    if (!n) {
      if (diet_filters.indexOf(diet) > -1) {
        ...
        setRecipeState({
          ...recipeState,
          diet_filters: [...diet_filters, diet],
        });
      }
    } else {
      if (cookbookEditState.diet_filters.indexOf(diet) > -1) {
        const filtered = cookbookEditState.diet_filters.filter(
          (item) => item !== diet
        );
        ...
        setCookbookEditState({
          ...cookbookEditState,
          diet_filters: [...cookbookEditState.diet_filters, diet],
        });
      }
    }
  };
  ...
  const handleChangeToEdit = () => {
    if (!edit) {
      ...
      const temp_description = currentRecipeState.description
        ? currentRecipeState.description
        : "";
      const temp_steps = currentRecipeState.steps
        ? currentRecipeState.steps
        : ([] as string[]);
      const temp_ingredients = currentRecipeState.ingredients
        ? currentRecipeState.ingredients
        : ([] as string[]);
      const diet_filters = currentRecipeState.diet_filters
        ? currentRecipeState.diet_filters
        : ([] as string[]);
      ...
      setRecipeState({
        ...
        description: temp_description,
        steps: temp_steps,
        ingredients: temp_ingredients as string[],
        step: "",
        ingredient: "",
        diet_filters: diet_filters,
      });
  ...
  const handleCookbookEditSubmit = () => {
    dispatch(
      editCookbook(auth, { cookbook: cookbookEditState }, "editCookbook")
    );
  ...
  useEffect(() => {
    ...
    if (location.pathname.substring(9, index) === auth.user!._id) {
      const c = auth.user!.cookbook.find(
        (x: any) => x._id === location.pathname.substring(index + 10)
      );
      setCookbookState({ own: true, cookbook: c });
    } else if (location.pathname.substring(9, index) == profile._id) {
      const c = profile.cookbook.find(
        (x: any) => x._id === location.pathname.substring(index + 10)
      );
  ...
  return (
    <React.Fragment>
      <Grid container>
        <Grid item xs={12} sx={{ pl: 10, ml: 10, pt: 5, mt: 10, pb: 0 }}>
          <Typography variant="h2">{cookbook.title}</Typography>
        </Grid>
        <Grid item xs={12} sx={{ pl: 10, ml: 10, pt: 0, mt: 10, pb: 5 }}>
          <Typography variant="h5">{cookbook.description}</Typography>
        </Grid>
      </Grid>
      ...
      <Container>
          ...
          >
                  ...
                  <React.Fragment>
                    ...
                    >
                      <ImageButton
                        ...
                        key={`IMAGE${n._id}`}
                        style={{ width: 700 }}
                        onClick={() => handleDialogOpen(1, n, index)}
                      >
                        <ImageSrc
                        ...
                        {!edit ? (
                                  ...
                                  <React.Fragment>
                                    ...
                                    <Grid xs={12}>
                                      <List dense className={classes.root}>
                                        {currentRecipeState.ingredients.map(
                                          (value) => {
                          ...
                          <div>
                            <form onSubmit={handleSubmitRecipeEdit}>
                              ...
                              <DialogContent>
                                ...
                                <Grid container>
                                  ...
                                  <Grid item xs={2}>
                                    <Button
                                      onClick={() => handleIngredientChange(-1)}
                                      color="primary"
                                    >
                                      <AddCircleIcon
                                        sx={{ pt: 2 }}
                            ...
                            <DialogActions>
                              ...
                              <Button
                                onClick={() => handleSubmitRecipeEdit()}
                                color="primary"
                              >
                                Submit Edit
                              </Button>
            ...
            {own ? (
              ...
              >
                ...
                >
                  <ImageSrcNew
                    style={{ backgroundImage: `url(${imageBackCookbook})` }}
                  />
                  <ImageBackdrop className="MuiImageBackdrop-root" />
                            ...
                            return (
                              <Grid item xs={12}>
                                <ListItem
                                  ...
                                  button
                                >
                                  <ListItemText id={labelId} sx={{ pl: 8 }}>
                                    {" "}
                                    <Typography variant="h5">
                                      {" "}
                                      {value}
        ...
        {own ? (
          <React.Fragment>
            ...
            <Dialog
              open={edit_cookbook_dialog}
              onClose={() => handleDialogOpen(3)}
              aria-labelledby="form-dialog-title"
            >
              <form onSubmit={handleCookbookEditSubmit}>
                ...
                <DialogActions>
                  ...
                  <Button
                    onClick={() => handleCookbookEditSubmit()}
                    color="primary"
                  >
                    Edit Cookbook
                  </Button>
        ...
        <Button
          sx={{ mt: 10, ml: 2 }}
          onClick={() => {
            navigate(`/profile/${own ? auth.user!._id : profile._id}`);
          }}
          color="primary"
          variant="outlined"
        >
